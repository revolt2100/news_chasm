...
input_filename = 'sentiment_objects_values.json'
output_filename = 'transfromed_modality.json'
...
for article_index, article in enumerate(dataset):
    ...
    all_image_items_in_article = [] 

    # --- B. PROCESS EVERY IMAGE ---
    # FIX: Guard against 'null' Images arrays
    for image_index, image in enumerate(article.get('Images') or []):
        image_items = []
        
        # FIX: The "or []" ensures that if it is 'null' (None), it becomes an empty list
        for obj in (image.get('image_objects') or []):
            if isinstance(obj, dict) and 'name' in obj:
                image_items.append(obj['name'])

        for concept in (image.get('image_concepts_list') or []):
